// Apply mig 156 (Processing Calendar domain) to TEST via exec_sql, then
...
const MILESTONE_ID = 'pmile-156-' + STAMP;
const BATCH_GID = 'gid-156-' + STAMP;
let batchId = null;
let subId = null;
let failures = 0;
...
(async () => {
  ...
  const body = fs.readFileSync(
    path.join(__dirname, '..', 'supabase-migrations', '156_processing_calendar.sql'),
    'utf8',
  );
  console.log(`applying 156_processing_calendar.sql (${body.length} bytes)`);
