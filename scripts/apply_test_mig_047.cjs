// Applier for migration 047 (submit_equipment_fueling RPC + historical
// reconciliation) against the test Supabase project. Mirrors
// apply_test_mig_034.cjs / apply_test_mig_035.cjs:
...
// exec_sql.
//
// The strip regex requires the trailing semicolon — bare `BEGIN` / `END`
// keywords appear inside plpgsql function bodies and must NOT be stripped.
//
// Idempotent — CREATE OR REPLACE FUNCTION + the reconciliation CTE's
// "only update when latest > current" guard. Re-running is a no-op.
//
// Usage: node scripts/apply_test_mig_047.cjs
...
(async () => {
  ...
  const file = '047_equipment_fueling_rpc.sql';
