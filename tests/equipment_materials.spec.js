...
async function seedEquipment(supabaseAdmin, overrides = {}) {
  const id = overrides.id || `eq-mat-${Math.random().toString(36).slice(2, 8)}`;
  const row = {
    id,
    name: overrides.name || 'Mat Test Tractor',
    slug: overrides.slug || `mat-${Math.random().toString(36).slice(2, 8)}`,
    category: 'tractors',
    status: 'active',
    tracking_unit: 'hours',
    current_hours: 80,
...
async function seedMaterial(supabaseAdmin, equipment_id, overrides = {}) {
  const id = overrides.id || `esm-mat-${Math.random().toString(36).slice(2, 8)}`;
  const row = {
    id,
    equipment_id,
  ...
  const {error} = await supabaseAdmin.from('equipment_service_materials').insert(row);
...
test('overdue piece appears; Clear hides only that material', async ({page, supabaseAdmin, resetDb}) => {
  ...
  const m1 = await seedMaterial(supabaseAdmin, eq.id, {id: 'esm-grease', material_name: 'Grease', sort_order: 10});
...
test('home dashboard shows due materials, clears one without affecting siblings, persists on refresh', async ({
  ...
  await resetDb();
  const eq = await seedEquipment(supabaseAdmin, {slug: 'mat-home', name: 'MatHome', current_hours: 80});
  const m1 = await seedMaterial(supabaseAdmin, eq.id, {id: 'esm-home-grease', material_name: 'Grease'});
  const m2 = await seedMaterial(supabaseAdmin, eq.id, {
    id: 'esm-home-loctite',
    material_name: 'Loctite 567',
    sort_order: 20,
...
test('clear expires when next_due milestone advances', async ({page, supabaseAdmin, resetDb}) => {
  ...
  const m = await seedMaterial(supabaseAdmin, eq.id, {id: 'esm-cross-grease'});

  // Pre-insert a clear in bucket=50 (overdue, never-completed).
  await supabaseAdmin.from('equipment_material_clears').insert({
    id: 'emc-pre',
    material_id: m.id,
    equipment_id: eq.id,
    due_bucket_value: 50,
