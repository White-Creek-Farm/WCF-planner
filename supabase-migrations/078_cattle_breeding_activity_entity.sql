-- Migration 078: Add cattle.breeding to the _activity_can_read resolver.
--
-- Custom editable-table Activity Phase B: scopes cattle breeding-cycle
-- create/edit/delete audit events to a breeding-workflow entity
-- (cattle.breeding). Like cattle.forecast it is a singleton workflow/table
-- entity (entity_id = 'cattle-breeding') with no per-row existence check —
-- readability is gated purely on cattle program access (admins always; users
-- with no program restriction; otherwise the 'cattle' program).
--
-- This migration replaces the full _activity_can_read function to preserve
-- every existing branch (including cattle.forecast from mig 076) and add the
-- new one before the fail-closed default.
